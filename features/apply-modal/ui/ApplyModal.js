...
const COPY = {
  ru: {
    ...
    successMsg: 'Заявка отправлена! Мы свяжемся с вами в течение 24 часов.',
  },
  kg: {
    ...
    successMsg: 'Өтүнмө жөнөтүлдү! Биз 24 саат ичинде байланышабыз.',
  },
  en: {
    ...
    successMsg: 'Application submitted! We will contact you within 24 hours.',
  },
}
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="640px" title={c.title}>
      ...
      <div className={styles.actions}>
        {step > 0 && <button className={styles.btnBack} onClick={back}>{c.back}</button>}
        {step < 2 ? (
          <button className={styles.btnNext} onClick={next}>{c.next}</button>
        ) : (
          <button className={styles.btnSubmit} onClick={submit}>{c.submit}</button>
        )}
      </div>
    </Modal>
  )
